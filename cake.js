...
function cake(recipe, available){
    ...
    for(key in recipeKey){
        if(availableKey.includes(key) === -1) return 0;
    }
    let answer = infinity;
    recipeKey.forEach(recikey => {
        if(available[recikey]/recipe[recikey] <= answer){
            answer = available[recikey] /recipe[recikey];
        }
    })
